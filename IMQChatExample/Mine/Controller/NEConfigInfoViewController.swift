...
/// 配置信息本地存储 Keys
struct QChatConfigInfoKeys {
  static let appKey = "qchat_config_appKey"
  static let account = "qchat_config_account"
  static let token = "qchat_config_token"
  static let openclawAccount = "qchat_config_openclawAccount"
}
...
public class NEConfigInfoViewController: NEBaseViewController {
  ...
  func loadSavedConfig() {
    ...
    appKeyTextField.text = ud.string(forKey: QChatConfigInfoKeys.appKey)
    accountTextField.text = ud.string(forKey: QChatConfigInfoKeys.account)
    tokenTextField.text = ud.string(forKey: QChatConfigInfoKeys.token)
    openclawTextField.text = ud.string(forKey: QChatConfigInfoKeys.openclawAccount)
  }
  ...
  func saveConfig() {
    ...
    let ud = UserDefaults.standard
    ud.set(appKeyText, forKey: QChatConfigInfoKeys.appKey)
    ud.set(accountText, forKey: QChatConfigInfoKeys.account)
    ud.set(tokenText, forKey: QChatConfigInfoKeys.token)
    ud.set(openclawText, forKey: QChatConfigInfoKeys.openclawAccount)
    ud.synchronize()
  ...
  @objc func resetConfig() {
    ...
    alert.addAction(UIAlertAction(title: "确定重置", style: .destructive, handler: { [weak self] _ in
      // 清空 UserDefaults
      let ud = UserDefaults.standard
      ud.removeObject(forKey: QChatConfigInfoKeys.appKey)
      ud.removeObject(forKey: QChatConfigInfoKeys.account)
      ud.removeObject(forKey: QChatConfigInfoKeys.token)
      ud.removeObject(forKey: QChatConfigInfoKeys.openclawAccount)
      ud.synchronize()
  ...
  override open func backEvent() {
    ...
    // 检测是否有未保存的变更
    let ud = UserDefaults.standard
    let savedAppKey = ud.string(forKey: QChatConfigInfoKeys.appKey) ?? ""
    let savedAccount = ud.string(forKey: QChatConfigInfoKeys.account) ?? ""
    let savedToken = ud.string(forKey: QChatConfigInfoKeys.token) ?? ""
    let savedOpenclaw = ud.string(forKey: QChatConfigInfoKeys.openclawAccount) ?? ""
