...
class QChatGroupIdentityMemberCell: UITableViewCell {
  ...
  public var memberModel: ServerMemeber? {
    didSet {
      ...
      if let name = model.nick,!name.isEmpty {
        avatarImage.configHeadData(headUrl: model.avatar, name: name, uid: model.accid ?? "")
      } else {
        avatarImage.configHeadData(headUrl: model.avatar, name: model.accid ?? "", uid: model.accid ?? "")
      }
  ...
  func setupSubviews() {
    ...
    NSLayoutConstraint.activate([
      ...
      arrowImageView.leftAnchor.constraint(
    ...
    for i in 0 ..< dataArray.count {
      ...
      if (maxWidth - labelsWidth) >= labelSize.width, isFirstRow {
        ...
        NSLayoutConstraint.activate([
          ...
          label.widthAnchor.constraint(equalToConstant: min(labelSize.width, kScreenWidth - labelsWidth - kScreenInterval)),
          label.heightAnchor.constraint(equalToConstant: labelSize.height),
        ])
      }
