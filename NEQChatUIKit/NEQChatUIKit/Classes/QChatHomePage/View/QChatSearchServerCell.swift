...
class QChatSearchServerCell: UITableViewCell {
  typealias CallBack = (() -> Void)?
  @objc var joinServerCallBack: CallBack = nil
  private let className = "QChatSearchServerCell"
...
extension QChatSearchServerCell {
  @objc func bottomBtnClick(sender: UIButton) {
    ...
    repo.applyServerJoin(parameter: param) { [self] error in
      ...
      if let err = error {
        UIApplication.shared.keyWindow?.makeToast(localizable("failed_operation"), duration: 2, position: .center)
      } else {
        joinBtn.isHidden = true
        rightContent.isHidden = false
        if joinServerCallBack != nil {
          joinServerCallBack!()
        }
      }
    }
