...
//          self?.rolesData.timeTag = self?.rolesData.roles.last?.role?.createTime
          ...
          placeholdModel.title = localizable("see_more") + "(" + localizable("total") + "\(roles.count)" + localizable("items") + ")"
          placeholdModel.isPlacehold = true
          if let lmt = self?.limit,
             roles.count >= lmt {
            self?.rolesLimitData.roles.append(contentsOf: self?.rolesData.roles.prefix(lmt) ?? [])
            self?.rolesLimitData.roles.append(placeholdModel)
          } else {
            self?.rolesLimitData.roles.append(contentsOf: self?.rolesData.roles ?? [])
          }

          placeholdModel.title = localizable("pick_up") + "(" + "\(roles.count)" + localizable("items") + ")"
          placeholdModel.isPlacehold = true
      ...
      if error != nil {
        ...
        if let members = memberRoles, members.count > 0 {
          ...
          if members.count == self?.rolesData.pageSize {
            var placeholdModel = RoleModel()
            placeholdModel.title = localizable("see_more")
            placeholdModel.isPlacehold = true
  ...
  public func removeChannelRole(role: ChannelRole?, index: Int,
    ...
    repo.removeChannelRole(param: param) { anError in
      completion(anError)
    }
  }
